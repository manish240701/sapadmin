...
const Category10 = ({ data, id }) => {
  const categoryHeader = "Placement and Internship";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category10Data = data.filter((d) => {
    return d.activityCategory === categoryHeader;
  });

  useEffect(() => {
    const sum = category10Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category10Data.length;
