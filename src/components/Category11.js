...
const Category11 = ({ data, id }) => {
  const categoryHeader = "Entrepreneurship";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category11Data = data.filter((d) => {
    return d.activityCategory === categoryHeader;
  });

  useEffect(() => {
    const sum = category11Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category11Data.length;
