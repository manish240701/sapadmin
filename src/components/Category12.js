...
const Category12 = ({ data, id }) => {
  const categoryHeader = "Social Activities";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category12Data = data.filter((d) => {
    return d.activityCategory === categoryHeader;
  });

  useEffect(() => {
    const sum = category12Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category12Data.length;
