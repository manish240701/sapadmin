...
const Category3 = ({ data, id }) => {
  const categoryHeader = "Techno Managerial Events";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category3Data = data.filter((d) => {
    return d.activityCategory === categoryHeader;
  });

  useEffect(() => {
    const sum = category3Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category3Data.length;
