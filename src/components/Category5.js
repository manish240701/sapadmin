...
const Category5 = ({ data, id }) => {
  const categoryHeader = "Membership";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category5Data = data.filter((d) => {
  ...
  useEffect(() => {
    const sum = category5Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category5Data.length;
