...
const Category6 = ({ data, id }) => {
  const categoryHeader = "Leadership/Organizing Events";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category6Data = data.filter((d) => {
    return d.activityCategory === categoryHeader;
  });

  useEffect(() => {
    const sum = category6Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category6Data.length;
