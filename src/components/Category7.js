...
const Category7 = ({ data, id }) => {
  const categoryHeader = "VAC/Online Courses";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category7Data = data.filter((d) => {
    return d.activityCategory === categoryHeader;
  });

  useEffect(() => {
    const sum = category7Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category7Data.length;
