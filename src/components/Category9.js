...
const Category9 = ({ data, id }) => {
  const categoryHeader = "GATE/CAT/Govt Exams";
  const [categoryPoints, setCategoryPoints] = useState(0);
  const [categoryCount, setCategoryCount] = useState(0);
  const category9Data = data.filter((d) => {
  ...
  useEffect(() => {
    const sum = category9Data.reduce((acc, cur) => acc + cur.weightage, 0);
    const totalItems = category9Data.length;
  ...
  return (
    ...
    >
      <div className="category-header">
        <h4>
          {categoryHeader}
          <span style={{ fontSize: "15px" }}> (MAX - 1000 points)</span>
        </h4>
        <div
          className={
            category9Data.length > 0
